...
public class StraightFireParticle extends AbstractGameEffect {
    ...
    public StraightFireParticle(float x, float y, float maxHeight, int Ignite) {
        ...
        if(p().hasPower(SpontaneousHumanCombustionPower.POWER_ID) && Ignite == 0) { this.color = new Color(MathUtils.random(0.1F, 0.3F), MathUtils.random(0.2F, 0.4F), MathUtils.random(0.7F, 1.0F), 0.0F); }
    ...
    public void update() {
        ...
        if (1.0F - this.duration < 0.1F) { this.color.a = Interpolation.fade.apply(0.0F, 1.0F, (DUR - this.duration) * 10.0F);
    ...
    public void dispose() { }
    static { GRAVITY = 180.0F * Settings.scale; }
}
