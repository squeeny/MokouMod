...
public class BlueHemokinesisParticle extends AbstractGameEffect {
    ...
    public BlueHemokinesisParticle(float sX, float sY, float tX, float tY, boolean facingLeft) {
        this.img = ImageMaster.GLOW_SPARK_2;
        this.pos = new Vector2(sX, sY);
        if (!facingLeft) { this.target = new Vector2(tX + DST_THRESHOLD, tY);
        } else { this.target = new Vector2(tX - DST_THRESHOLD, tY); }
    ...
    private void updateMovement() {
        ...
        if (!this.stopRotating) {
            if (this.rotateClockwise) { this.rotation += Gdx.graphics.getDeltaTime() * this.rotationRate;
            } else {
        ...
        this.pos.sub(tmp);
        if (this.stopRotating) { this.currentSpeed += Gdx.graphics.getDeltaTime() * VELOCITY_RAMP_RATE * 3.0F;
        } else { this.currentSpeed += Gdx.graphics.getDeltaTime() * VELOCITY_RAMP_RATE * 1.5F; }
        if (this.currentSpeed > MAX_VELOCITY) { this.currentSpeed = MAX_VELOCITY; }
        if (this.target.dst(this.pos) < DST_THRESHOLD) {
            for (int i = 0; i < 5; i++) {
                if (this.facingLeft) { AbstractDungeon.effectsQueue.add(new DamageImpactLineEffect(this.target.x + DST_THRESHOLD, this.target.y));
                } else { AbstractDungeon.effectsQueue.add(new DamageImpactLineEffect(this.target.x - DST_THRESHOLD, this.target.y)); }
            }
        ...
        if (!this.controlPoints.isEmpty()) {
            if (!(this.controlPoints.get(0)).equals(this.pos)) { this.controlPoints.add(this.pos.cpy()); }
        } else { this.controlPoints.add(this.pos.cpy()); }
        ...
        if (this.controlPoints.size() > 10) { this.controlPoints.remove(0); }
