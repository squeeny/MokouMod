...
public class MoveImageWithSparkleEffect extends AbstractGameEffect {
    ...
    public MoveImageWithSparkleEffect(float srcX, float srcY, float destX, float destY, Texture img, Color col, String soundKey, AbstractGameEffect finalEffect) {
        ...
        if (this.sY > this.dY) { this.bounceHeight = 600.0F * Settings.scale;
        } else { this.bounceHeight = this.dY - this.sY + 600.0F * Settings.scale; }
    }

    public void update() {
        if (!this.playedSfx) {
            this.playedSfx = true;
            if(soundKey != null) { CardCrawlGame.sound.playA(soundKey, MathUtils.random(-0.5F, 1.0F)); }
        }
        ...
        if (this.dX > this.sX) { this.rotation -= Gdx.graphics.getDeltaTime() * 1000.0F;
        } else { this.rotation += Gdx.graphics.getDeltaTime() * 1000.0F;
        }
        ...
        } else { this.yOffset = Interpolation.circleOut.apply(0.0F, this.bounceHeight, this.duration / 0.25F) * Settings.scale; }
        this.duration -= Gdx.graphics.getDeltaTime();
        if (this.duration < 0.0F) {
            if(finalEffect != null) { AbstractDungeon.effectsQueue.add(finalEffect); }
            this.isDone = true;
        }
    }
